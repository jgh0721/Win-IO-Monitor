...
void EncryptBufferByAES( LONGLONG Offset, PVOID Buffer, ULONG BufferSize, PVOID Key, ULONG KeyLength, PVOID IV, ULONG IVLength )
{
	...
	nsDetail::aes_set_key( &context, ( uint8_t* )Key, KeyLength == 16 ? 128 : KeyLength == 24 ? 192 : KeyLength == 32 ? 256 : 0 );
	// NOTE: AES 의 블럭크기는 암호화 키 길이에 관계없이 항상 128 bit( 16 byte ) 이다
	for( int idx = 0; idx < count; ++idx )
		nsDetail::aes_encrypt_ecb( &context, ( uint8_t* )Add2Ptr( Buffer, idx * 16 ), ( uint8_t* )Add2Ptr( Buffer, idx * 16 ) );
}
...
void DecryptBufferByAES( LONGLONG Offset, PVOID Buffer, ULONG BufferSize, PVOID Key, ULONG KeyLength, PVOID IV, ULONG IVLength )
{
	...
	// NOTE: AES 의 블럭크기는 암호화 키 길이에 관계없이 항상 128 bit( 16 byte ) 이다
	for( int idx = 0; idx < count; ++idx )
		nsDetail::aes_decrypt_ecb( &context, ( uint8_t* )Add2Ptr( Buffer, idx * 16 ), ( uint8_t* )Add2Ptr( Buffer, idx * 16 ) );
}
